        ...
        <div>
          ...
          <nav className="space-y-1 px-3">
            {NAV_ITEMS.map((item) => {
              ...
              const isActive = pathname.startsWith(item.href) || (pathname === "/dashboard/plan" && item.id === "dashboard");
              return (
                <Link
                  key={item.id}
                  href={item.href}
        ...
        <header className="z-20 flex h-14 w-full flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6 dark:border-white/5 dark:bg-[#0e1628] md:px-8">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => router.push("/dashboard/profile")} className="flex items-center gap-3 cursor-pointer" type="button">
              ...
              <div className="flex size-8 items-center justify-center rounded-full border border-slate-200 bg-indigo-500 font-sans text-xs font-black text-white dark:border-white/5">
