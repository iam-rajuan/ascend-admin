...
const NAV_ITEMS = [
  { id: "handoffs", label: "IDMT Handoffs", icon: ClipboardCheck, href: "/dashboard/idmt/handoffs" },
];

export function IdmtLayout({ children }: { children: React.ReactNode }) {
        ...
        <div>
          ...
          <nav className="space-y-1 px-3">
            {NAV_ITEMS.map((item) => {
              ...
              const isActive = pathname.startsWith(item.href) || pathname === "/dashboard/idmt";
              return (
                <Link
                  key={item.id}
                  href={item.href}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="z-20 flex h-14 w-full flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6 dark:border-white/5 dark:bg-[#0e1628] md:px-8">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">IDMT Clinical Handoffs</span>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => router.push("/dashboard/profile")} className="flex items-center gap-3 cursor-pointer" type="button">
              ...
              <div className="flex size-8 items-center justify-center rounded-full border border-slate-200 bg-teal-500 font-sans text-xs font-black text-white dark:border-white/5">
        ...
        <div className="z-10 flex h-6 w-full flex-shrink-0 items-center justify-center border-b border-slate-800 bg-slate-900 px-6 font-sans text-[9px] font-mono tracking-wider text-slate-500 select-none">
          ...
          CUI // HIPAA Protected · IDMT Transmitted Handoff Summaries
        </div>
