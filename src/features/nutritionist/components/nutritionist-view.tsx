  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {activeTab === "dashboard" && (
        <div className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard title="Active Nutrition Cases" value={formatNumber(dashboard?.active_reconditioning_count)} subtext="Active nutrition cases." />
            <MetricCard title="Pending Review" value={formatNumber(dashboard?.pending_review_total)} subtext="Records needing review." accent="text-amber-500" />
            <MetricCard title="Unread Messages" value={formatNumber(threads?.threads?.length)} subtext="Active message threads." />
          </div>
        </div>
      )}
      ...
      {activeTab === "consults" && (
        <div className="space-y-6">
          <Card>
            <CardHeader title="Nutrition Consultations" subtitle="Schedule and record dietary consultations." />
      ...
      {activeTab === "records" && (
        <div className="space-y-6">
          <Card>
            <CardHeader title="Dietary & Nutritional Records" subtitle="Clinical dietary logs and uploaded records." />
            <div className="space-y-4 text-xs">
              <p className="text-slate-500">Records count: {records?.records ? records.records.length : 0}</p>
            </div>
          </Card>
