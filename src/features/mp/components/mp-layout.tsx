...
const NAV_ITEMS = [
  { id: "dashboard", label: "Dashboard", icon: TrendingUp, href: "/dashboard/mp/dashboard" },
  { id: "notes", label: "Clinical Notes", icon: FileText, href: "/dashboard/mp/notes" },
  { id: "records", label: "Mental Health Records", icon: Shield, href: "/dashboard/mp/records" },
  { id: "messages", label: "Messages", icon: MessageSquare, href: "/dashboard/mp/messages" },
];
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#f0f4f9] font-sans text-slate-800 transition-colors duration-200 dark:bg-[#070a13] dark:text-slate-100">
        ...
        <div>
          ...
          <nav className="space-y-1 px-3">
            {NAV_ITEMS.map((item) => {
              ...
              const isActive = pathname.startsWith(item.href) || (pathname === "/dashboard/mp" && item.id === "dashboard");
              return (
                <Link
                  key={item.id}
                  href={item.href}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="z-20 flex h-14 w-full flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6 dark:border-white/5 dark:bg-[#0e1628] md:px-8">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => router.push("/dashboard/profile")} className="flex items-center gap-3 cursor-pointer" type="button">
              ...
              <div className="flex size-8 items-center justify-center rounded-full border border-slate-200 bg-rose-500 font-sans text-xs font-black text-white dark:border-white/5">
        ...
        <div className="z-10 flex h-6 w-full flex-shrink-0 items-center justify-center border-b border-slate-800 bg-slate-900 px-6 font-sans text-[9px] font-mono tracking-wider text-slate-500 select-none">
          ...
          CUI // HIPAA & Behavioral Health Privacy Protected
        </div>
